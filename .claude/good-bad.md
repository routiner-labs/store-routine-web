...
- 모든 페이지/컴포넌트는 다크 모드까지 기본으로 지원해야 한다. 색은 반드시 CSS 토큰(`--color-*`)만 쓰고 표면/배경/텍스트/보더 색 하드코딩 금지, 옅은 배경·보더가 필요하면 rgba 틴트/`color-mix` 방식 사용. 작업 후 시스템 설정(사이드바 하단 이름/아바타 클릭 → 설정)에서 다크 모드를 켜고 실제 확인까지 해야 완료 — 상세 규칙은 `agents/design-agent.md`의 "다크 모드" 섹션 참고.
